...
class DenseNet(tn.Module):
    ...
    def __init__(self, indim=1, outdim=1, hidden_units=None, name2Model='DNN', actName2in='tanh', actName='tanh',
        ...
        if str.lower(self.name2Model) == 'fourier_dnn':
            ...
            self.dnn_layers.append(input_layer)
            ...
            for i_layer in range(len(hidden_units)-1):
                if i_layer == 0:
                    ...
                    tn.init.xavier_normal_(hidden_layer.weight)
                    tn.init.uniform_(hidden_layer.bias, -1, 1)
                self.dnn_layers.append(hidden_layer)
            ...
            self.dnn_layers.append(out_layer)
        else:
    ...
    def forward(self, inputs, scale=None, sFourier=0.5):
        ...
        for i_layer in range(0, len(self.hidden_units)-1):
            H_pre = H
            dense_layer = self.dnn_layers[i_layer+1]
            # print(dense_layer)
            H = dense_layer(H)
            H = self.actFunc(H)
...
class Pure_FullyNet(tn.Module):
    ...
    def __init__(self, indim=1, outdim=1, hidden_units=None, name2Model='DNN', actName2in='tanh', actName='tanh',
        ...
        super(Pure_FullyNet, self).__init__()
        self.to_gpu = to_gpu
        self.gpu_no = gpu_no
        self.indim = indim
        self.outdim = outdim
...
class Fully_ScaleNet(tn.Module):
    """
    Args:
        indim: the dimension for input data
        outdim: the dimension for output
    ...
    def __init__(self, indim=1, outdim=1, hidden_units=None, name2Model='DNN', actName2in='tanh', actName='tanh',
                 actName2out='linear', scope2W='Weight', scope2B='Bias', repeat_Highfreq=True, type2float='float32',
                 to_gpu=False, gpu_no=0):
        super(Fully_ScaleNet, self).__init__()
        self.indim = indim
        self.outdim = outdim
        self.to_gpu = to_gpu
        self.gpu_no = gpu_no
...
class Fourier_FeatureDNN(tn.Module):
    ...
    def forward(self, inputs, scale=None, sFourier=1.0, training=None, mask=None):
        ...
        for i_layer in range(0, len(self.hidden_units) - 1):
            H_pre = H
            dense_layer = self.dnn_layers[i_layer]
            H = dense_layer(H)
            H = self.actFunc(H)
            if self.hidden_units[i_layer + 1] == hidden_record:
                H = H + H_pre
...
class Fully_FourierNet(tn.Module):
    """
    Args:
        indim: the dimension for input data
        outdim: the dimension for output
    ...
    def __init__(self, indim=1, outdim=1, hidden_units=None, name2Model='DNN', actName2in='tanh', actName='tanh',
        ...
        super(Fully_FourierNet, self).__init__()
        self.indim = indim
        self.outdim = outdim
        self.to_gpu = to_gpu
        self.gpu_no = gpu_no
...
class DNN2TEST(tn.Module):
    def __init__(self, input_dim=2, out_dim=1, hidden_layer=None, Model_name='DNN', name2actIn='tanh',
        ...
        if str.upper(Model_name) == 'DNN':
            self.DNN = Pure_FullyNet(indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name,
            ...
            self.DNN = Fully_ScaleNet(indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name,
                                      actName2in=name2actIn, actName=name2actHidden, actName2out=name2actOut,
                                      type2float=type2numeric, to_gpu=use_gpu, gpu_no=no2gpu,
            ...
            self.DNN = Fully_FourierNet(indim=input_dim, outdim=out_dim, hidden_units=hidden_layer, name2Model=Model_name,
                                        actName2in=name2actIn, actName=name2actHidden, actName2out=name2actOut,
                                        type2float=type2numeric, to_gpu=use_gpu, gpu_no=no2gpu,
                                        repeat_Highfreq=repeat_Highfreq)
