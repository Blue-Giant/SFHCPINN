...
def save_trainLoss2mat(data2loss, name2loss=None, actName=None, outPath=None):
    outFile2data = '%s/%s2%s.mat' % (outPath, name2loss, actName)
    key2mat = str(name2loss)
    scio.savemat(outFile2data, {key2mat: data2loss})
...
def save_2testGD(model_G, model_D, PINN, outPath=None):
    outFile2data = '%s/test_GD.mat' % (outPath)
    key2mat_1 = 'Model_G'
    key2mat_2 = 'Model_D'
    key2mat_3 = 'PINN'
    scio.savemat(outFile2data, {key2mat_1: model_G, key2mat_2: model_D, key2mat_3: PINN})
