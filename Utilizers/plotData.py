...
def plotTrain_loss_1act_func(data2loss, lossType=None, outPath=None, xaxis_scale=False, yaxis_scale=False):
    plt.figure()
    ax = plt.gca()
...
def plotTrain_MSE_REL_1act_func(data2mse, data2rel, actName=None, outPath=None, xaxis_scale=False,
                                yaxis_scale=False):
    plt.figure()
    ax = plt.gca()
    ...
    fntmp = '%s/trainErr_%s' % (outPath, str(actName))
    DNN_tools.mySaveFig(plt, fntmp, ax=ax, isax=1, iseps=0)
...
def plotTest_MSE_REL(data2mse, data2rel, epoches, actName=None, outPath=None, xaxis_scale=False, yaxis_scale=False):
    plt.figure()
    ax = plt.gca()
    ...
    fntmp = '%s/testERR_%s' % (outPath, actName)
    DNN_tools.mySaveFig(plt, fntmp, ax=ax, isax=1, iseps=0)
...
def plot_Hot_solution2test(solu2test, size_vec2mat=20, actName=None, seedNo=1000, outPath=None):
    ...
    plt.imshow(solu2color, interpolation='nearest', cmap='RdBu', origin='lower')
    ...
    if str.lower(actName) == 'utrue':
        ...
        fntmp = '%s/%s_%s' % (outPath, seedNo, 'Us-SinAddcos')
    else:
        fntmp = '%s/%s_%s' % (outPath, seedNo, 'Save')
    DNN_tools.mySaveFig(plt, fntmp, ax=ax, isax=1, iseps=0)
...
def plotTrain_learningrate(data2loss, lossType=None, seedNo=1000, outPath=None, xaxis_scale=False, yaxis_scale=False):
    plt.figure()
    ax = plt.gca()
    plt.plot(data2loss, 'b-.', label=lossType)
    plt.xlabel('epoch', fontsize=14)
