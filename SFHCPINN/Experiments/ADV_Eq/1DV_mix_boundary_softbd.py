...
def f_side2smooth(x, t, ws=0.001, ds=0.002, alpha=0.001, beta=20, zeta=1.0, PI=torch.pi):
    ...
    pu_pt = -alpha*exp_t*torch.sin(2.0*PI*x)
    pu_px = exp_t*(2.0*PI*torch.cos(2.0*PI*x))
    ppu_pxx = exp_t*(-4.0*PI*PI*torch.sin(2.0*PI*x))
...
def f_side_case_2pi_Beta_pi(x, t, ws=0.001, ds=0.002, alpha=0.001, beta=20, zeta=0.01, PI=torch.pi):
    exp_t = torch.exp(-alpha*t)
    sin_2pi_beta_pi = torch.sin(2.0 * PI * x) + zeta*torch.sin(beta * PI * x)
    pu_pt = -alpha*exp_t*sin_2pi_beta_pi

    item2pu_px = 2.0 * PI * torch.cos(2.0*PI*x) + zeta * beta * PI * torch.cos(beta * PI * x)
    pu_px = exp_t*item2pu_px

    item2ppu_pxx = -4.0 * PI * PI * torch.sin(2.0 * PI * x) - zeta * beta * PI * beta * PI * torch.sin(beta * PI * x)
    ppu_pxx = exp_t * item2ppu_pxx
    f_side = pu_pt - ds*ppu_pxx + ws*pu_px
    return f_side
...
def solve_ADV_PDE(R):
    ...
    if R['equa_name'] == 'ADE1d_Sin(2pi)':
        ...
        u_init = lambda x, t: torch.mul(torch.exp(-alpha * t), torch.sin(2.0*pi * x))
        u_left = lambda x, t: torch.mul(torch.exp(-alpha * t), torch.sin(2.0 * pi * x))
        u_right = lambda x, t: 2.0 * pi * torch.mul(torch.exp(-alpha * t), torch.cos(2.0 * pi * x))

        u_true = lambda x, t: torch.mul(torch.exp(-alpha * t), torch.sin(2.0*pi * x))
    elif R['equa_name'] == 'ADE1d_Sin(pi)+0.1sin(10Pi)':
        region_l = 0.0
        region_r = 2.0
        init_time = 0.0
        end_time = 5.0
    ...
    elif R['equa_name'] == 'ADE1d_Sin(2pi)+0.1sin(10Pi)':
        region_l = 0.0
        region_r = 1.0
        init_time = 0.0
        end_time = 1.0
        ...
        beta = 10
        zeta = 0.1
        f_side = lambda x, t: f_side_case_2pi_Beta_pi(x=x, t=t, ws=ws, ds=ds, alpha=alpha, beta=beta, zeta=zeta)

        u_init = lambda x, t: torch.mul(torch.exp(-alpha*t), torch.sin(2*pi*x) + zeta*torch.sin(beta*pi*x))
    ...
    if R['testData_model'] == 'random_generate':
        ...
        if R['equa_name'] == 'ADE1d_Sin(pi)+0.1sin(20Pi)':
            mat_data_path = '../OneDimMatData2ADE/ST_02_05/'
        elif R['equa_name'] == R['equa_name'] == 'ADE1d_Sin(2pi)':
    ...
    xr_bd_batch.requires_grad = True
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        loss = loss_it + loss_bd2left * temp_penalty_bd + R['bd_penalty2Neumann']*loss_bd2right + loss_init * temp_penalty_init
...
if __name__ == "__main__":
    ...
    if R['model2NN'] == 'Fourier_SubDNN':
        store_file = 'SoftFourierSub2MixBD_1D'
    elif R['model2NN'] == 'Fourier_DNN':
        store_file = 'SoftFourier2MixBD_1D'
    elif R['model2NN'] == 'DNN':
        store_file = 'SoftDNN2MIxBD_1D'
    ...
    # R['equa_name'] = 'ADE1d_Sin(pi)+0.1sin(20Pi)'
    # R['equa_name'] = 'ADE1d_Sin(2pi)+0.1sin(10Pi)'   # used for paper
