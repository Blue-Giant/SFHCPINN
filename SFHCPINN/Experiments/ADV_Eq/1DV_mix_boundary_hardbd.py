...
def solve_ADV_PDE(R):
    ...
    # 左边狄利克雷边界， 右边诺伊曼
    if R['equa_name'] == 'ADE1d_Sin(2pi)_Analytical_D_G':
        region_l = 0.0
        region_r = 1.0
        init_time = 0.0
        end_time = 1.0
        ...
        u_left = lambda x, t: torch.mul(torch.exp(-alpha * t), torch.sin(2*pi * x))
        u_right = lambda x, t: 2.0*pi*torch.mul(torch.exp(-alpha * t), torch.cos(2 * pi * x))

        model_D = lambda x, t: torch.mul(temp1(x) / np.square(region_r - region_l), t / end_time)
        model_G = lambda x, t: torch.sin(2*pi*x)
        # model_G = lambda x, t: torch.sin(1.5 * pi * x)
        # model_G = lambda x, t: torch.sin(3*pi * x)
        # model_G = lambda x, t: -1.0*torch.multiply(torch.cos(2 * pi * x), torch.ones_like(t))
        # model_G = lambda x, t: torch.multiply(torch.cos(pi * x), torch.ones_like(t))
        # model_G = lambda x, t: torch.multiply(torch.sin(1.5*pi * x), torch.ones_like(t))
        # model_G = lambda x, t: x*(x-0.5)+(x-0.5)*(1-x)
        # model_G = lambda x, t: x * (1 - x)
        # model_G = lambda x, t: 0.5*torch.sin(pi * x) + 0.5*torch.sin(2*pi * x)
        # model_G = lambda x, t: 0.5 * torch.sin(pi * x) + 0.5 * torch.cos(1.5*pi * x)
    elif R['equa_name'] == 'ADE1d_Sin(pi)+0.1sin(20Pi)_Analytical_D_G':
        region_l = 0.0
        region_r = 2.0
        init_time = 0.0
        end_time = 5
        ...
        u_right = lambda x, t: torch.mul(torch.exp(-alpha*t), pi*torch.cos(pi*x) + zeta*beta*pi*torch.cos(beta*pi*x))
    elif R['equa_name'] == 'ADE1d_Sin(2pi)+0.1sin(20Pi)_Analytical_D_G':
        region_l = 0.0
        region_r = 1.0
        init_time = 0.0
        end_time = 1.0
        ...
        u_left = lambda x, t: torch.mul(torch.exp(-alpha*t), torch.sin(2*pi*x)+zeta*torch.sin(beta*pi*x))
        u_right = lambda x, t: torch.mul(torch.exp(-alpha*t), 2*pi*torch.cos(2*pi*x)+zeta*beta*pi*torch.cos(beta*pi*x))
        ...
        # model_G = lambda x, t: torch.sin(pi * x) + zeta * torch.sin(10 * pi * x)
        ...
        model_G = lambda x, t: torch.multiply(zeta*torch.sin(beta * pi * x), torch.ones_like(t))
    ...
    if R['testData_model'] == 'random_generate':
        ...
        elif R['equa_name'] == 'ADE1d_Sin(2pi)_Analytical_D_G' or R['equa_name'] == 'ADE1d_Sin(2pi)_NN_D_G':
    ...
    x_rbd_batch = torch.ones(size=[batchsize_bd, 1]) * region_r
    if True == R['use_gpu']:
        x_rbd_batch = x_rbd_batch.cuda(device='cuda:' + str(R['gpuNo']))
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        if R['equa_name'] == 'ADE1d_Sin(2pi)+0.1sin(50Pi)_Analytical_D_G' or \
            ...
            loss_bd = model.loss2Neu(X_bd=x_rbd_batch, T_bd=t_bd_batch, Ubd_exact=u_right, type2boundary='right',
        ...
        loss = loss_in + bd_penalty_init*loss_bd
...
if __name__ == "__main__":
    ...
    if R['model2NN'] == 'Fourier_SubDNN':
        store_file = 'HardFourierSub2MixBD_1D'
    elif R['model2NN'] == 'Fourier_DNN':
        store_file = 'HardFourier2MixBD_1D'
    ...
    # R['equa_name'] = 'ADE1d_Sin(pi)+0.1sin(20Pi)_Analytical_D_G'
    # R['equa_name'] = 'ADE1d_Sin(2pi)+0.1sin(20Pi)_Analytical_D_G'  # used for paper

    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Setup of DNN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    # 训练集的设置(内部和边界)
    R['batch_size2interior'] = 2000  # 内部训练数据的批大小
    R['batch_size2boundary'] = 250  # 边界训练数据的批大小
