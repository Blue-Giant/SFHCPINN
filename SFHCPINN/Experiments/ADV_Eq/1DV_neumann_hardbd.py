...
def EX8(z, t, ws, ds):
    temp1 = torch.div((z - ws * t), torch.sqrt(4 * ds *t))
    temp3 = torch.div((z + ws * t), torch.sqrt(4 * ds *t))
    temp2 = torch.exp(ws * z /ds)
    temp1 = erfc1(temp1)
    temp3 = erfc1(temp3)
    label = 0.5 * temp1 + 0.5 * torch.mul(temp3, temp2.cpu())
    return label
...
class HardPINN(tn.Module):
    ...
    def loss_1DV(self, X_in=None, T_in=None, loss_type='l2_loss', scale2lncosh=0.5, ws=None, ds=None, force_side=None,
        ...
        # dD/dx dD/dy ddD/dxy ddD/dxx ddD/dyy
        ...
        if dGNN2dt is None:
            dGNN2dt = 0
        # dx dy dxx dyy
        dGNN2dx = torch.autograd.grad(GNN, X_in, grad_outputs=torch.ones_like(X_in),
                                      create_graph=True, retain_graph=True)[0]
        ...
        if dGNN2dx is None:
            ...
            ddGNN2dxx = torch.autograd.grad(dGNN2dx, X_in, grad_outputs=torch.ones_like(X_in),
                                            create_graph=True, retain_graph=True)[0]
        if ddGNN2dxx is None:
            ddGNN2dxx = 0

        # loss
        dt = dGNN2dt + torch.mul(dDNN2dt, UNN) + torch.mul(DNN, dUNN2t)
        ...
        res = dt + ws * dx - ds * dxx - Fside

        HbdNN = GNN + torch.mul(DNN, UNN)
        ...
        return HbdNN, loss_it

    def loss2Neu(self, X_bd=None, T_bd=None, Ubd_exact=None, if_lambda2Ubd=True, loss_type='l2_loss', scale2lncosh=0.1,
        ...
        XT = torch.mul(X_bd, self.mat2X) + torch.mul(T_bd, self.mat2T)
        if if_lambda2Ubd:
        ...
        UNN = self.DNN(XT, scale=self.factor2freq, sFourier=self.sFourier)
        dUNN2dx = torch.autograd.grad(UNN, X_bd, grad_outputs=torch.ones_like(X_bd), create_graph=True,
                                      retain_graph=True)[0]
        ...
        if True == if_NN2model_G:
            ...
            GNN = model2Gene(X_bd, T_bd)

        # dG/dx
        dGNN2dx = torch.autograd.grad(GNN, X_bd, grad_outputs=torch.ones_like(X_bd), create_graph=True,
        ...
        if True == if_NN2model_D:
            ...
            DNN = model2Dist(X_bd, T_bd)

        dDNN2dx = torch.autograd.grad(DNN, X_bd, grad_outputs=torch.ones_like(X_bd),
                                      create_graph=True, retain_graph=True, allow_unused=True)[0]
        if dDNN2dx is None:
            dDNN2dx = 0

        dHardNN2dx = dGNN2dx + torch.mul(dDNN2dx, UNN) + torch.mul(DNN, dUNN2dx)
        diff_bd = 1.0 * dHardNN2dx - U_bd
        if str.lower(loss_type) == 'l2_loss':
...
def solve_Multiscale_PDE(R):
    ...
    if R['equa_name'] == '1DV_General_Analytical_D_G':
        region_l = 0.0
        region_r = 2.0
        init_time = 0.0
        end_time = 5.0
        ...
        # model_D = lambda x, t: t / end_time
        # model_D = lambda x, t: torch.ones_like(t)-torch.exp(-t)
        model_G = lambda x, t: torch.sin(torch.pi*x)
    elif R['equa_name'] == '1DV_General_NN_D_G':
        region_l = 0.0
        region_r = 2.0
        init_time = 0.0
        end_time = 5.0
        ...
        model_G = torch.load('../PreModel2HardPINN/Neumann1D_ExtensionModel2Space02Time05.pth')
        model_D = torch.load('../PreModel2HardPINN/Neumann1D_DistModel2Space02Time05.pth')
    elif R['equa_name'] == '1DV_MultiScale_Analytical_D_G':
        region_l = 0.0
        region_r = 1.0
        init_time = 0.0
        end_time = 1.0
        ...
        # model_G = lambda x, t: torch.sin(torch.pi*x)+zeta*torch.sin(omega*torch.pi*x)
        model_G = lambda x, t: torch.sin(torch.pi * x)
    elif R['equa_name'] == '1DV_MultiScale_NN_D_G':
        region_l = 0.0
        region_r = 1.0
        init_time = 0
        end_time = 1
        ...
        u_init = lambda x, t: torch.sin(torch.pi*x)+zeta*torch.sin(omega*torch.pi*x)
        f_side = lambda x, t: get_fside2multi_scale(x, t, alpha=alpha, zeta=zeta, omega=omega, ws=ws, ds=ds)
        model_G = torch.load('../PreModel2HardPINN/Neumann1D_ExtensionModel2Space01Time01.pth')
        model_D = torch.load('../PreModel2HardPINN/Neumann1D_DistModel2Space01Time01.pth')
    else:
        region_l = 0.0
        region_r = 1.0
        init_time = 0
    ...
    if R['testData_model'] == 'random_generate':
        ...
        if R['equa_name'] == '1DV_General_Analytical_D_G' or \
                R['equa_name'] == '1DV_General_NN_D_G':
            mat_data_path = '../OneDimMatData2ADE/ST_02_05/'
        elif R['equa_name'] == '1DV_MultiScale_Analytical_D_G' or \
                R['equa_name'] == '1DV_MultiScale_NN_D_G':
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        if R['equa_name'] == '1DV_General_Analytical_D_G' or R['equa_name'] == '1DV_MultiScale_Analytical_D_G':
            UNN2train, loss_it = model.loss_1DV(X_in=x_in_batch, T_in=t_in_batch, loss_type=R['loss_type'], ws=ws,
            ...
            loss_lbd = model.loss2Neu(X_bd=x_lbd_batch, T_bd=t_bd_batch, Ubd_exact=u_left, type2boundary='left',
                                      loss_type='l2_loss', model2Dist=model_D, model2Gene=model_G, if_NN2model_D=False,
                                      if_NN2model_G=False)
            loss_rbd = model.loss2Neu(X_bd=x_rbd_batch, T_bd=t_bd_batch, Ubd_exact=u_right, type2boundary='right',
                                      loss_type='l2_loss', model2Dist=model_D, model2Gene=model_G, if_NN2model_D=False,
                                      if_NN2model_G=False)
        else:
            UNN2train, loss_it = model.loss_1DV(X_in=x_in_batch, T_in=t_in_batch, loss_type=R['loss_type'], ws=ws,
            ...
            loss_lbd = model.loss2Neu(X_bd=x_lbd_batch, T_bd=t_bd_batch, Ubd_exact=u_left, type2boundary='left',
                                      loss_type='l2_loss', model2Dist=model_D, model2Gene=model_G, if_NN2model_D=True,
                                      if_NN2model_G=True)
            loss_rbd = model.loss2Neu(X_bd=x_rbd_batch, T_bd=t_bd_batch, Ubd_exact=u_right, type2boundary='right',
                                      loss_type='l2_loss', model2Dist=model_D, model2Gene=model_G, if_NN2model_D=True,
                                      if_NN2model_G=True)

        loss_bd = loss_lbd + loss_rbd
        ...
        if i_epoch % 1000 == 0:
            ...
            DNN_tools.print_and_log_train_one_epoch_pinn2hard(
                i_epoch, run_times, tmp_lr, loss_it.item(), loss_bd.item(), loss.item(), train_mse.item(),
...
if __name__ == "__main__":
    ...
    if R['model2NN'] == 'Fourier_SubDNN':
        store_file = 'HardFourierSub2Neumann_1D'
    elif R['model2NN'] == 'Fourier_DNN':
        store_file = 'HardFourier2Neumann_1D'
    ...
    R['PDE_type'] = 'PINN'
    # R['equa_name'] = '1DV_General_Analytical_D_G'
    # R['equa_name'] = '1DV_General_NN_D_G'

    R['equa_name'] = '1DV_MultiScale_Analytical_D_G'
    # R['equa_name'] = '1DV_MultiScale_NN_D_G'
