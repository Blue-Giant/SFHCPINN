...
def print_and_log_test_one_epoch(mse2test, rel2test, log_out=None):
    # 将运行结果打印出来
...
def print_and_log_train_one_epoch_1(i_epoch, run_time, tmp_lr, loss_it_tmp,
                                    loss_bd_tmp, loss_tmp, train_mse_tmp, train_res_tmp, log_out=None):
    # 将运行结果打印出来
    print('train epoch: %d, time: %.3f' % (i_epoch, run_time))
    print('learning rate: %f' % tmp_lr)
...
def print_and_log_train_one_epoch_pinn(i_epoch, run_time, tmp_lr, loss_it_tmp,loss_bd_tmp,loss_init_tmp,
...
def print_and_log_train_one_epoch_pinn_1(i_epoch, run_time, tmp_lr, temp_penalty_bd, temp_penalty_init, loss_it_tmp,
                                         loss_bd_tmp, loss_init_tmp, loss_tmp, train_mse_tmp, train_res_tmp,
                                         log_out=None):
    # 将运行结果打印出来
    print('train epoch: %d, time: %.3f' % (i_epoch, run_time))
    print('learning rate: %.10f' % tmp_lr)
    print('boundary penalty: %f' % temp_penalty_bd)
    print('Penalty for init: %f' % temp_penalty_init)
    print('loss_it for training: %.10f' % loss_it_tmp)
    print('loss_bd for training: %.10f' % loss_bd_tmp)
    print('loss_init for training: %.10f' % loss_init_tmp)
    print('loss for training: %.10f' % loss_tmp)
    ...
    log_string('boundary penalty: %f' % temp_penalty_bd, log_out)
    log_string('Penalty for init: %f' % temp_penalty_init, log_out)
    log_string('loss_it for training: %.10f' % loss_it_tmp, log_out)
    log_string('loss_bd for training: %.10f' % loss_bd_tmp, log_out)
    log_string('loss_init for training: %.10f' % loss_init_tmp, log_out)
    log_string('loss for training: %.10f' % loss_tmp, log_out)
    log_string('solution mean square error for training: %.10f' % train_mse_tmp, log_out)
    log_string('solution residual error for training: %.10f\n' % train_res_tmp, log_out)


def print_and_log_train_one_epoch_pinn2hard(i_epoch, run_time, tmp_lr, loss_it_tmp, loss_bd_tmp,
                                            loss_tmp, train_mse_tmp, train_res_tmp, log_out=None):
    # 将运行结果打印出来
    print('train epoch: %d, time: %.3f' % (i_epoch, run_time))
    print('learning rate: %f' % tmp_lr)
    ...
    print('loss for training: %.10f' % loss_tmp)
    print('solution mean square error for training: %.10f' % train_mse_tmp)
    print('solution relative error for training: %.10f\n' % train_res_tmp)
    ...
    log_string('loss for training: %.10f' % loss_tmp, log_out)
    log_string('solution mean square error for training: %.10f' % train_mse_tmp, log_out)
    log_string('solution relative error for training: %.10f\n' % train_res_tmp, log_out)

def print_and_log_train_one_epoch_pinn2hard_EX1(i_epoch, run_time, tmp_lr, loss_it_tmp,
                                                loss_tmp, train_mse_tmp, train_res_tmp, log_out=None):
        # 将运行结果打印出来
        print('train epoch: %d, time: %.3f' % (i_epoch, run_time))
        print('learning rate: %f' % tmp_lr)
        print('loss_it for training: %.15f' % loss_it_tmp)
        print('loss for training: %.10f' % loss_tmp)
        ...
        log_string('train epoch: %d,time: %.3f' % (i_epoch, run_time), log_out)
        log_string('learning rate: %f' % tmp_lr, log_out)
        log_string('loss_it for training: %.15f' % loss_it_tmp, log_out)
        log_string('loss for training: %.15f' % loss_tmp, log_out)
...
def print_and_log_train_one_epoch2Ocean(i_epoch, run_time, tmp_lr, temp_penalty_bd, temp_penalty_init, pwb,
    ...
    log_string('learning rate: %.10f' % tmp_lr, log_out)
    log_string('boundary penalty: %f' % temp_penalty_bd, log_out)
    log_string('init penalty: %f' % temp_penalty_init, log_out)
    log_string('weights and biases with  penalty: %f' % pwb, log_out)
    log_string('loss_it for training: %.10f' % loss_it_tmp, log_out)
    log_string('loss_bd for training: %.10f' % loss_bd_tmp, log_out)
    log_string('loss_init for training: %.10f' % loss_init, log_out)
    log_string('loss for training: %.10f' % loss_tmp, log_out)
    log_string('solution mean square error for training: %.10f' % train_mse_tmp, log_out)
