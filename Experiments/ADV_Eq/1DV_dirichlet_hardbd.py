...
class HardPINN(tn.Module):
    ...
    def loss_1ADV(self, X_in=None, T_in=None, loss_type='l2_loss', scale2lncosh=0.5, ws=0.001, ds=0.002, force_side=None,
        ...
        # dt = torch.mul(dDNN2dt, UNN) + torch.mul(DNN, dUNN2dt)
...
def solve_ADV_PDE(R):
    ...
    if R['equa_name'] == 'General_ADE':  # 双诺依曼边界
        ...
        temp1 = lambda x: torch.mul(x, region_r - x)
        f_side = lambda x, t: f_side2sin_pi_x_sin_50_pi_x(x=x, t=t, ws=ws, ds=ds, alpha=alpha, beta=beta, zeta=zeta)
        model_D = lambda x, t: torch.mul(x*(2.0-x), t/end_time)
        model_G = lambda x, t: torch.sin(pi * x) + zeta * torch.sin(beta * pi * x)
...
if __name__ == "__main__":
    ...
    # &&&&&&&&&&&&&&&&&&&&&& 隐藏层的层数和每层神经元数目 &&&&&&&&&&&&&&&&&&&&&&&&&&&&
    if R['model2NN'] == 'Fourier_SubDNN':
        # R['hidden_layers'] = (10, 25, 20, 20)  # （1*10+250+500+400+300+15）* 20 = 1475 *20 (subnet个数) = 29500
        R['hidden_layers'] = (10, 25, 20, 20, 10)  # （1*10+250+500+400+300+15）* 20 = 1475 *20 (subnet个数) = 29500
        # R['hidden_layers'] = (10, 25, 10)  # （1*10+250+500+400+300+15）* 20 = 1475 *20 (subnet个数) = 29500
        # R['hidden_layers'] = (10, 25, 20)  # （1*10+250+500+400+300+15）* 20 = 1475 *20 (subnet个数) = 29500
        # R['hidden_layers'] = (20, 25, 10)  # （1*10+250+500+400+300+15）* 20 = 1475 *20 (subnet个数) = 29500
